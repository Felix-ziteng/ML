...
# Define the paths
path_local = "/home/cristiano/Documents/pulp-sdk/tests/qsort/"
...
with open("5_reg_machine_learning_malware_results.log", "w") as log_file:
  
  # Loop through all quadruples of files
  for files_comb in itertools.combinations(file_names, 5):
      ...
      subprocess.check_call([path_encrypt_learn + "5d10000sel_enc"] + [path_encrypt_learn + f for f in files_comb])
      
      # Run the second command : noencrypt learn
      subprocess.check_call([path_noencrypt_learn + "5d10000sel_noenc"] + [path_noencrypt_learn + f for f in files_comb])
      
      # Run the third command : encrypt test
      subprocess.check_call([path_encrypt_test + "5d_test_enc"] + [path_encrypt_test + f for f in files_comb])

      # Run the fourth command : noencrypt test
      subprocess.check_call([path_noencrypt_test + "5d_test_noenc"] + [path_noencrypt_test + f for f in files_comb])
      
      # Print the results of the last command to the log
      result = subprocess.check_output(["python3", path_local + "ml_2d.py"])
